...
class MinimaxPlayer:
    ...
    def state_score_reachable_slots(self, board, loc):
        ...
        # if self.loc == (4, 5) or self.loc == (3, 5):
        #     print(f"checking from (4,5), reachable from {loc}: {nodes_set}\n")
        #     print("\t total reachable: ", len(nodes_set))
    ...
    def make_move(self, time):  # time parameter is not used, we assume we have enough time.
        ...
        while (time_until_now + next_iteration_max_time + 0.5 < time) and d <= (len(self.board) * len(self.board[0])):
            d += 1

...
class AlphaBetaPlayer(MinimaxPlayer):
    ...
    def make_move(self, time):
        ...
        leafs_l = [0]
        d = 1
        move, leafs = self.Alpha_Beta(board=self.board, depth=d, leafs=leafs_l, start_time=ID_start_time, time_lim=time,
        ...
        if turn == OUR_TURN:
            ...
            for d in self.directions:
                ...
                if 0 <= i < len(board) and 0 <= j < len(board[0]) and board[i][j] == 0:  # then move is legal
                    new_loc = (i, j)
                    board[new_loc] = 1
                    score = self.Alpha_Beta_MinMax_Aux(board, depth - 1, leafs, new_loc, enemy_loc, ENEMY_TURN, alpha,
                                                       beta)
                    if score > best_score:
                        best_score = score
                    board[new_loc] = 0
        ...
        # enemy turn
            ...
            for d in self.directions:
                ...
                if 0 <= i < len(board) and 0 <= j < len(board[0]) and board[i][j] == 0:  # then move is legal
                    new_loc = (i, j)
                    board[new_loc] = 2
                    score = self.Alpha_Beta_MinMax_Aux(board, depth - 1, leafs, our_loc, new_loc, OUR_TURN, alpha, beta)
